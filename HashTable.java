...
public class HashTable<K, V> extends AbstractMap<K, V> {
  ...
  public V put (K key, V value) {
    ...
    Entry entryAdd = new Entry(key, value);
      int hashIndex = hashIndex(key);
    entryAdd.next = table[hashIndex];
      table[hashIndex] = entryAdd;
  ...
  private void rehash (int newCapacity) {
    ...
    for (int i = 0; i < oldTable.length; i++) {
      ...
      while(entry != null) {
        Entry entryAdd = new Entry(entry.key, entry.value);
        int hashIndex = hashIndex(entry.key);
        entryAdd.next = table[hashIndex];
        table[hashIndex] = entryAdd;
        entry = entry.next;
      }
  ...
  public V remove (Object key) {
   ...
   if(table[index]==null) {
    ...
    if(table[index].key == key) {
      Entry entry = table[index];
      Entry returnEntry = table[index];
      entry = entry.next;
      table[index] = entry;
      size--;
      if(((double)table.length / 4.0) >= size) {
